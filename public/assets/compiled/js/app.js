window.addEventListener("load", () => {
    ...
    const activeItemKey = "activeSidebarItem"; // Key for local storage
    ...
    const getCurrentPageIdentifier = () => {
        // Example using URL pathname
        return window.location.pathname;
        // Alternatively, use other identifiers based on your page structure
    };

    // Function to set the active class based on local storage or current page
    const setActiveFromStorageOrPage = () => {
        const currentPage = getCurrentPageIdentifier();
        let activeItemIndex = localStorage.getItem(activeItemKey);

        if (activeItemIndex !== null) {
            const index = parseInt(activeItemIndex, 10);
            const itemToActivate = sidebarItems[index];

            if (
                itemToActivate &&
                itemToActivate.getAttribute("data-path") === currentPage
            ) {
                removeActiveClasses();
        ...
        // No matching item, fallback: Default to first item if no other item is active
        ...
        if (!anyItemActive && sidebarItems.length > 0) {
    ...
    const handleItemClick = (index) => {
        localStorage.setItem(activeItemKey, index); // Save active item index to local storage
        // Directly update the sidebar without reloading
        setActiveFromStorageOrPage();
    };
